<template>
  <view class="canvas-display">
    ...
    <view v-if="!no_poem" class="return-button">
      <button @click="clearAll(true)" style="background-color: #666">重置</button>
    </view>
    ...
    <view v-if="no_poem" class="function-bar">
      ...
      <view class="upload-section">
        <button @click="dialog=true" style="background-color: #666">上传涂鸦</button>
        <view v-if="dialog" class="dialog">
          <view class="dialog-content">
            ...
            <button @click="selectImg" style="background-color: #666">保存上传</button>
          </view>
        </view>
      </view>
      ...
      <view class="submit-section">
        <button @click="submit_node" style="background-color: #666">提交搜索</button>
      </view>
      
      <!-- Clear -->
      <view v-if="!uploadSketch" class="clear-section">
        <button @click="clearCanvas"style="background-color: #666">清空画布</button>
        <button @click="startErase"style="background-color: #666">切换橡皮</button>
        <button @click="stopErase"style="background-color: #666">切换画笔</button>
    ...
    <view v-else class="results-display">
      ...
      <view class="poem-section">
        ...
        <swiper circular indicator-dots autoplay :interval="2000" style="height: 72vh">
          <swiper-item v-for="(info, index) in this.results.poems" :key="info.poetry_id">
            <!-- <button @click="navigateToPoetry(info.poetry_id)">{{info.name}}</button> -->
            <text>{{info.name}}{{ info.author }}</text>
